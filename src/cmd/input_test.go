...
func TestSetResourceOnStructWithSchemaUsingQuotedJSON(t *testing.T) {
	input := []byte(`
name: hello world
schema: "{\"age\": 50, \"active\": true}"
`)
...
func TestSetResourceOnStructWithSchemaUsingMultilineJSON(t *testing.T) {
	input := []byte(`
name: hello world
schema: |
  {
...
`)
	act, err := cmd.ReadResource[MockResourceInput](input)
	autopilot.Ok(t, err)
	exp := MockResourceInput{Name: "hello world", Schema: map[string]any{"name": "hello world", "active": true}}
	autopilot.Equals(t, exp, *act)
}
